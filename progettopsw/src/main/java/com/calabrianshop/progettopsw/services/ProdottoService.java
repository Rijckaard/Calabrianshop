...
public class ProdottoService {
    ...
    @Transactional(readOnly = false)
    public void updateQuantita (int prid){
        if(!prodottoRepository.existsById(prid))
            throw  new IllegalArgumentException("prodotto inesistente!");
        Prodotto up = em.find(Prodotto.class, prid);
        Integer old = up.getQuantita();
        up.setQuantita(old+1);
        em.flush();
    }
